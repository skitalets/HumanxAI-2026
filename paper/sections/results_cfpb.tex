...
Narratives classified as delegative scored significantly higher than evaluative narratives on all five moral foundations, with the largest effects for loyalty ($d = 0.256$, $p < .001$) and sanctity ($d = 0.255$, $p < .001$), followed by care ($d = 0.228$, $p < .001$). The loyalty result is particularly important: in the Moral Foundations framework, the loyalty foundation tracks in-group betrayal and allegiance --- precisely the relational vocabulary our theory associates with delegative trust violation.

\input{tables/table_cfpb_emfd}

Correlation patterns reinforce this interpretation (Table~\ref{tab:cfpb_emfd}). Delegative scores correlate positively with all foundations, most strongly with loyalty ($\rho = .118$, $p < .001$). Evaluative scores correlate \textit{negatively} with all foundations; for Sonnet, the strongest negative correlates are sanctity ($\rho = -.119$, $p < .001$) and care ($\rho = -.112$, $p < .001$). The opposing sign pattern --- positive delegative correlations, negative evaluative correlations across all five foundations --- holds for both Sonnet and GPT-4o classifications, providing further evidence that the two models are recovering the same construct distinction. The within-foundation ranking of effect sizes differs slightly across models (see Table~\ref{tab:cfpb_emfd}).
...
We interpret this pattern as follows. The dual-model approach functions as an automated coding system with known biases (over-attribution of evaluative intent in factual narratives). The high-confidence subset, where all three raters converge, represents the clearest cases of each trust mode. The analyses reported above are robust to restricting to concordant cases (see Table~\ref{tab:cfpb_gradient}, concordant rows). However, because the dual-model concordance establishes inter-rater \emph{reliability} rather than construct \emph{validity}, a shared tendency toward over-attribution would persist in concordant cases. The high-confidence subset, where all three raters (both models and the human coder) converge, provides the strongest evidence that the classifications track the intended constructs.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{delegative_evaluative_scatter.png}
